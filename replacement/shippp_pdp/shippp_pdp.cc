...
constexpr int first_RRPV  = 0;
constexpr int second_RRPV = 4;
constexpr int third_RRPV  = 3;
constexpr int fourth_RRPV = 7;
...
// find victim by priority
std::vector<int>::iterator find_victim_by_priority(std::vector<int>::iterator& begin, std::vector<int>::iterator& end){
    auto victim = std::find(begin, end, ::first_RRPV);
    if (victim == end){
        victim = std::find(begin, end, ::second_RRPV);
        if (victim == end){
            victim = std::find(begin, end, ::third_RRPV);
            if (victim == end){
                victim = std::find(begin, end, ::fourth_RRPV);
            }
        }
    }
    return victim;
...
uint32_t CACHE::find_victim(uint32_t triggering_cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
  ...
  auto victim = find_victim_by_priority(begin, end);
  while (victim == end) {
    for (auto it = begin; it != end; ++it)
      --(*it);

    victim = find_victim_by_priority(begin, end);
  }
                                     ...
                                     uint8_t hit)
{
  ...
  if (hit && access_type{type} != access_type::WRITE) {
    ...
    if (access_type{type} == access_type::PREFETCH &&  ::is_prefetch[this][set * NUM_WAY + way]) {
        ...
        for(uint32_t  i = 0; i < NUM_WAY; i++){
            SAT_DEC(::rrpv_values[this][set * NUM_WAY + i]);
        }
        // SHiP++ : Prefetch-Aware RRPV Updates(scenarios 1)
        if (::is_prefetch[this][set * NUM_WAY + way]){
            ::rrpv_values[this][set * NUM_WAY + way] = ::first_RRPV;
  ...
  // High-confidence
    ...
    ::rrpv_values[this][set * NUM_WAY + way] = third_RRPV;
  }
  else {
    ::rrpv_values[this][set * NUM_WAY + way] = ::first_RRPV + 1;
  }
